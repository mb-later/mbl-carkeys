...
function Process(action, start, tick, finish)
    print("al sana progbar")
    DisableControlAction(0, 289, true) -- VehicleMouseControlOverride
    TriggerEvent("qb-inventory:progres-bar", true)
	ActionStart()
    Action = action
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("Notification", "Önünde başka bir bar açıkken bir işlem yapamazsın.")
        end
    else
        TriggerEvent("Notification", "Şuanda yapamazsın.")
    end
end
...
function Cancel()
    ...
    -- TriggerEvent("qb-inventory:progres-bar", false)
end
